...
const checkTag = res => {
    if(res.guild.tag != undefined || res.guild.tag != null) return res.guild.tag;
    else return `No tag`;
};
...
getInfoBtn.onclick = (e) => {
    ...
    if (tempName != '') {
        const guildPlayer = document.querySelectorAll('.guild_player');
        guildPlayer.forEach(data => {
            if (tempName == data.dataset.username) data.classList.remove('inGuild');
        })
    };
    document.querySelector('.guild_lead').scrollIntoView({block: "start", behavior: "smooth"});
...
getData(`https://api.vimeworld.ru/guild/get?id=29338`).then((result) => {
            for(let i = 0; i != result.members.length; i++){
                countMembers(result.members[i]);
                guildLead.innerHTML += `
                <div class="guild_player ${checkGuildRank(result.members[i].status, "LEADER")}" data-username="${result.members[i].user.username.toLowerCase()}">
                <a class="${checkStatus(result.members[i].user.rank)} hover" href="${`https://vimetop.ru/player/${result.members[i].user.username}`}" target="_blank"><img class="player_img" src="${`http://skin.vimeworld.ru/head/${result.members[i].user.username}.png`}" alt="da"><br>${result.members[i].user.username}</a>
                    <span>id - ${result.members[i].user.id}</span>
                </div>`
                guildOfficers.innerHTML += `
                <div class="guild_player ${checkGuildRank(result.members[i].status, "OFFICER")}" data-username="${result.members[i].user.username.toLowerCase()}">
                    <a class="${checkStatus(result.members[i].user.rank)} hover" href="${`https://vimetop.ru/player/${result.members[i].user.username}`}" target="_blank"><img class="player_img" src="${`http://skin.vimeworld.ru/head/${result.members[i].user.username}.png`}" alt="da"><br>${result.members[i].user.username}</a>
                    <span>id - ${result.members[i].user.id}</span>
                </div>`
                guildPlayers.innerHTML += `
                <div class="guild_player ${checkGuildRank(result.members[i].status, "MEMBER")}" data-username="${result.members[i].user.username.toLowerCase()}">
                    <a class="${checkStatus(result.members[i].user.rank)} hover" href="${`https://vimetop.ru/player/${result.members[i].user.username}`}" target="_blank"><img class="player_img" src="${`http://skin.vimeworld.ru/head/${result.members[i].user.username}.png`}" alt="da"><br>${result.members[i].user.username}</a>
                    <span>id - ${result.members[i].user.id}</span>
                </div>`
            }
